...
\begin{figure}
      ...
      \caption{Illustration of how different distribution-learning methods try to generate samples
            that resemble the data in distribution. String-based autoregressive models sample the
            next token according to a probability distribution which is based on the previously
            sampled tokens. The likelihood of the samples can be optimized using the probability
            chain rule. The generator in generative adversarial networks is trained to generate
            samples, that a discriminator cannot distinguish from real samples. Variational
            autoencoders first transform an input sample into a latent space and then reconstruct
            the input from the latent representation. Flow-based models make use of an invertible
            mapping which allows to calculate and maximize the exact likelihood of the data. }
\end{figure}
